...
Here I'm going to demonstrate how Stitch can be used to to expose APIs, so you can shield your development community from the nuances of your backend.
...
### Setting up MongoDB
To begin, log into [MongoDB Atlas](https://cloud.mongodb.com). If you don't already have an account, you can create one for free. Feel free to use an existing cluster for this exercise, but everything I do here will be on the free tier sandbox, which I named **FreeTier**.

![](images/free-tier.png)

### Loading Data
Download the [products.json](sample-data/products.json) sample data set. 
...
Open a terminal shell on your workstation at the directory where you downloaded the [products.json](sample-data/products.json) file and paste the copied mongoimport command. Set the `<PASSWORD>` to your password, `<DATABASE>` to **store**, the `<COLLECTION>` to **products**, the `<FILETYPE>` to **json** and the FILENAME to **products.json**. You can delete the `--type <FILETYPE>`. Here's an example for my cluster:
...
Run the command to import the product data in to MongoDB.
...
In Atlas, select Stitch from the left navigation menu:
...
Stitch provides powerful security controls, which I'll address in a future exercise. For now, let's turn on Anonymous Authentication and provide a MongoDB Collection for Stitch to work with:
...
In the Function Editor, replace the code with the following:
    ...
    // Fetch all records if no filter provided
    var filter = ((typeof arg !== 'undefined') ? {'name':arg} : {});
    console.log('Filter used: ' + filter.name);
...
Since we are only using this key for internal testing, you don't have to worry about copying the value, but do so if it makes you feel comforatable:

![](images/internal-testing-key-value.png)

While we are here, let's create a second API key for our web client, named **WebClient**. I like using seperate keys to distinguise who did what in the logs. In this case, you do want to copy the key, as we'll need it to connect from the web client later.
...
![](images/select-internal-testinguser.png)
...
Select **mongodb-atlas** under under ATLAS CLUSTERS on the left:
...
Save App.js. React automatically reloads the application everytime it detects a change. If you open your browser's JavaScript console, you can verify that you logged into Stitch successfully.
...
Beneath the constructor of the FilterableProductTable, add the following  getData function to call our API:
    ...
    getData() {
        ...
        stitchClient.executeFunction("getProductsByName")
            ).then(result => {
...
Then update the `handleFilterTextChange` method to call `getData()`:

    handleFilterTextChange(filterText) {
        this.getData().then(products => this.setState({ products: products, filterText: filterText }))
        .catch(e => {
            console.log('error', e)
        });
    }
